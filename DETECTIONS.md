...
| `vscode_tunnel_agent` | T1572 | Detects VS Code tunnel activity in AI-agent process trees (`code tunnel`, `code-tunnel`, `code serve-web`, `.vscode-server`). | Orange | Proofpoint reporting on TA415 VS Code tunnel abuse and MITRE T1572: https://www.proofpoint.com/ and https://attack.mitre.org/techniques/T1572/ | `code tunnel --accept-server-license-terms` |
| `llm_model_download` | T1105 | Detects download-tool usage (`curl`, `wget`, `aria2c`, `axel`) against common model-hosting domains (`huggingface.co`, `ollama.com`, `civitai.com`, etc.). | Orange | Recorded Future and vx-underground reporting on malware downloading local models; MITRE T1105: https://www.recordedfuture.com/ and https://www.vx-underground.org/ and https://attack.mitre.org/techniques/T1105/ | `wget https://huggingface.co/.../model.gguf` |
| `cloud_api_c2` | T1102 | Detects non-browser process access to Google Sheets/Drive/Docs API endpoints often abused for web-service C2 channels. | Orange | GC2 tradecraft and MITRE T1102 (Web Service): https://attack.mitre.org/techniques/T1102/ | `curl https://sheets.googleapis.com/v4/spreadsheets/...` |
| `tempdir_installer_abuse` | T1218 | Detects installer-like commands (`dpkg`, `rpm`, `pip install`, `npm install`, `cargo install`, `install.sh`) executed from temporary paths (`/tmp`, `/dev/shm`, etc.). | Orange | BYOI installer-abuse research and MITRE T1218: https://attack.mitre.org/techniques/T1218/ | `cwd=/tmp/stage && pip install evil-wheel.whl` |
